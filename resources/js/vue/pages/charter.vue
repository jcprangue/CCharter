            ...
            <div class="row">
                <div class="col-md-4">
                    <select class="form-control rounded-pill m-0 border border-success" v-model="searchText">
                        ...
                        <option v-for="(list,index) in autocomplete" :key="index" :value='list.service_name'>{{list.service_name}}</option>
                    </select>
...
export default {
    ...
    data() {
        return {
            dtData: {
                data: [],
                header: ["Service Name", "Steps", "Requirement","Forms Used", "Processing Time", "Service Provider"],
                key: ["service_name","steps","requirements","forms_used","processing_time", "service_provider"],
                th_size: [
                    "15%",
                    "25%",
                    "20%",
    ...
    watch: {
        code:function(val){
            this.$http.post("/charter/find/",{id:this.id,service_code:val}).then(data =>{
        ...
        searchText:function(val){
            var vm = this;
            vm.$http.post("/search/service",{id:vm.id,search:val}).then(data =>{
    ...
    methods: {
        getdeptData:function(){
            this.$http.post("/charter/find/",{id:this.id,service_code:this.code}).then(data =>{
                this.dtData.data = data.data;
