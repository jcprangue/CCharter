            ...
            <div class="row mt-4">
                <div class="col-md-3 border-right overflow-auto">
                    <ul class="list-group list-group-flush" style="font-size:10px;" >
                        <li class="list-group-item" v-for="(department, index) in dept" :key="index" @click="changesearch(department.id)">{{ department.department }}</li>
                    </ul>
                </div>
                <div class="col-md-9">
                     ...
                     <CustomTable :data="dtData" tableClass="table" :search_input="false">
                        <template slot="service_name" slot-scope="props" >
                             ...
                             <div class="searchResult">
                                <h3 class="mb-4 "><router-link class="text-green" :to="{name:'Charter', params:{id: props['row']['dept'],code:props['row']['service_code'] }}">{{ props["row"]["service_name"] }}</router-link> </h3>
                                <p>{{ props["row"]["steps"] }}</p>
                            </div>
                        </template>
                    </CustomTable>
